...
func addTwoNumbers(_ l1: ListNode?, _ l2: ListNode?) -> ListNode? {
    ...
    var next: ListNode?
    if sum > 9 {
        // 进1 把1加到下个节点的val, 构建next节点
        next = ListNode((l1?.next?.val ?? 0) + (sum / 10), l1?.next?.next)
        // 取余
        sum = sum % 10
    }
    
    // 递归构建next节点
    return ListNode(sum, addTwoNumbers(next, l2?.next))
}
// 迭代法
...
// 递归法合并
func merge(_ lists: [ListNode?], _ lo: Int, _ hi: Int) -> ListNode? {
